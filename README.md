# 성능 테스트 보고서  
**대상 서비스 / API**: 쿠폰 발급 API (예: `/coupon/apply`)  
**테스트 도구**: k6 (버전: …)  
**테스트 날짜**: YYYY-MM-DD  
...
| 항목 | 결과 |
|------|------|
| 총 요청 (http_reqs) | 197,088 건 |
| 성공률 (checks_succeeded) | 100.00% (197,088 / 197,088) |
| 실패율 (http_req_failed) | 0.00% |
| 평균 응답시간 (avg latency) | **250.22 ms** |
| 중앙값 응답시간 (median, p50) | 235.07 ms |
| p(90) 응답시간 | 313.50 ms |
| p(95) 응답시간 | 344.71 ms |
| 최대 응답시간 (max) | 555.74 ms |
| Throughput (요청/sec, 평균) | 약 19,320 req/s |
...
- 대부분 요청에 대해 **응답시간 250 ms 내외**, p95 기준 **약 345 ms 이하** 유지 — 상당히 양호한 응답 성능.  
...
- p95 또는 max 응답시간이 300~500 ms 구간인 만큼, 사용자 체감 기준(예: 200 ms 이하) 대상이라면 추가 최적화 여지 있음.  
...
이번 부하 테스트 결과는, **고부하 + 동시 요청** 상황에서도 시스템이 **안정성과 성능**을 충분히 잘 유지하고 있다는 것을 보여주는 좋은 증거야.  
이 리포트를 포트폴리오에 포함하면 — “단순 기능 구현 + 설계”뿐 아니라 “실제로 부하 테스트 + 성능 검증 + 안정성 확인”까지 한 개발자라는 인상을 줄 수 있어.  

다만, 지금은 “단순 + 짧은 burst 테스트” 결과이므로 — 실제 운영 조건 + 장시간 부하 + 복합 요청 시나리오 포함한 **추가 테스트 + 모니터링**이 뒷받침되면, 훨씬 더 설득력 있는 “무적급 성능 보고서”가 될 거야.  
